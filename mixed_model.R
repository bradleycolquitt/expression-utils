plot_v_pos_rand = function(coef_data, main_data, ps, n, dir="abs", randoms=1, gene_ids=NULL,
  ...
  tmp = main_data %>% 
    dplyr::filter(position==ps, gene_id %in% tp$gene_id)
  
  gg = ggplot(tmp, aes_string(slope_name, y_name, color=group_name)) + geom_point() + facet_grid(gene_id~tags, scales="free_y")
  ...
  gg = gg + scale_x_continuous(expand=c(0,200))
  if (ps=="ra") { 
...
plot_coefs = function(coef_data, ps, n, dir="abs",
  ...
  gg_coef %+% tp2 + labs(title=toupper(ps))
}
...
position_lme = function(db_name, db_table, value_name, variance_thresh, to_test=NULL,
  ...
  system.time({
    mods = lapply(to_test, function(pos) {
      ...
      inf = info2 %>% filter(position3==pos)
  ...
  out = list(call=right_side, regions=to_test, variance_thresh=variance_thresh, data=mods2)
  saveRDS(out, paste(prefix, model, sep="/"))

  
  models_file_fname = paste(prefix, "models_info.txt", sep="/")
  cnames = c("datetime", "call", "regions", "variance_thresh", "database", "database_table")
  new_entry = data.frame(datetime = now(),
                         database = db_name,
