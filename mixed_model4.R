...
plot_v_pos_rand = function(coef_data, main_data, ps, n, dir="abs", randoms=1, gene_ids=NULL,
  ...
  gg = ggplot(tmp, aes_string(slope_name, y_name, shape=group_name, color="position3")) + geom_point() + facet_grid(gene_id~tags, scales="free")
...
plot_mixed_combined = function(ps, 
  ...
  ggs = lapply(1:length(models), function(i) {
    gg = plot_coefs(models[[i]], ps, sum_n, slope_name=slope_name)
    gg = gg + labs(title=names(models)[i])
  })
  fname = paste(ps, "coef_summaries.pdf", sep="-")
  ...
  for (i in 1:length(models)) {
    fname = paste(paste(c(ps, c(model_info[i,c("database_table", "value_name", "shortcall")]), direction), collapse="-"), ".pdf", sep="")
    fname = paste(fig_path, fname, sep="/")
    print(fname)
    if (save) 
      cairo_pdf(fname)
    ...
    if (model_info[i,"shortcall"]=="fixSlope_randInterSlope" )
      randoms = 2
    gg = plot_v_pos_rand(models[[i]], main_data[[i]], ps, regress_n, direction, randoms=randoms, 
                         slope_name=slope_name, group_name = group_name)
    gg = gg + labs(title=names(models)[i])
    print(gg)
    if (save)
      dev.off()
...
position_lme_load_data = function(db_name, db_table, value_name) {
  ...
  info = data.frame(collect(tbl(db, "db_info")))
...
position_lme = function(db_name, db_table, value_name, variance_thresh, to_test=NULL,
  ...
  system.time({
    mods = lapply(to_test, function(pos) {
      ...
      inf = info2 %>% filter(reference2_position3==pos)
      if (nrow(inf)==0) {
        print("position3 not found")
        return(NULL)
      }
  ...
  #save = T
  prefix = paste("/media/data2/rstudio/birds/topography/models/mixed/", str_replace(db_name, ".db", ""), sep="/")
  dir.create(prefix)
